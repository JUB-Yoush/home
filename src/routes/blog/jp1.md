...
The first thing you'll notice about Japanese is that the squiggles we're used to deriving meaning from are gone. Instead are wildly different looking runes we must decipher. The alphabet we know is gone, and there is no alphabet to replace it. Instead, Japanese is built on a syllabary.
...
K + A = か (KA)
N + A = な (NA)
かな = Kana!!! (Wow it's been 3 minutes and you can already read Japanese!?)
...
There are in total: 85,000 Kanji according in [the history of all time forever](https://www.sljfaq.org/afaq/how-many-Kanji.html).
...
But Kanji do have their use, and you do gotta learn them to be literate to any capacity.

Another not-in-english issue we run into with these Kanji is how to look them up?

Each Kanji is unique so We don't have the luxury of just typing in the characters we see unlike with unfamiliar english words

some Kanji are written with with furigana (the hirigana is written on top), which makes looking them up easy, but they usually include have that.

Kanji are generally organized by stroke count, but that just puts them in groups, and you'd also need to be familiar with the stroke count of the Kanji, and if you're not familiar with it then you probably don't know how many strokes it'd take to write.
...
So the word for "Person" would be 人＝ひと, because the Kanji is by itself it uses it's Kun reading.
The word for "Black Person" would be 黒+人=こく+じん, because it's two Kanji together they both use their On readings

So you need to memorize 2136 * 2 readings? Not nessicarily. Some Kanji thankfully have the same Kun and On Readings, while some have multiple different Kun and On Readings, the Kanji for life (生) has 20 different ways it could be read depending on the word it's in.
...
People (人人 although it would actually be written 人々 because　々 repeats the Kanji to it's left)　You think would be じんじん, but not only does it use the Kun reading, ひとひと, but the second ひと get's rendaku'ed to ひとびと.
...
But then why is language learning so difficult for so many people? It's because our brains only treat languages as languages when *we* treat them as languages, not just as a hobbies, but the means in which we must communicate ideas.

Immersion is great for language learning, not from absorbing the language though osmosis, but because you have the opportunity to treat the language as the primary means of communication. There are lots of people who live in Tokyo for years and don't pick up on any Japanese, because of the abundance of english, they don't need to ever treat Japanese like a language (Tokyo might be the easiest place to navigate without knowing the language in the entire world).
...
- All video games/Anime/Manga must be consumed in Japanese.
- Follow Blogs or Content creators who only make content in your language.
- Make a Dungeons and Dragons party of other Japanese language learners and only play in that language (I actually am interested in doing this, not nearly good enough yet, but HMU if you're also interested)
- Follow blogs or YouTubers that only post in Japanese.
- Go to language exchanges and make friends that only communicate in Japanese.
